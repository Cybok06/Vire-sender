...
export function getAdminUsersSms(search = '', page = 1) {
  return request(`/api/admin/users-sms?${new URLSearchParams({ search, page: String(page) })}`, { headers: authHeaders() });
}

export function adjustAdminUserSms(userId, adjustment) {
  return request(`/api/admin/users-sms/${userId}/adjust`, {
    method: 'POST', headers: authHeaders(), body: JSON.stringify(adjustment),
  });
}
